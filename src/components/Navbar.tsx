  ...
  return (
    <nav className="sticky top-0 z-50" style={{ background: 'rgba(9,24,48,0.85)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
      <div className="mx-auto flex max-w-[1240px] items-center justify-between px-5 py-3">
        <a href="#" className="font-display text-[30px] leading-none tracking-wide select-none">
        ...
        <a href="#nieuwsbrief" className="hidden rounded-full bg-lime px-5 py-2 font-body text-sm font-bold text-primary-foreground transition-colors hover:bg-lime-dim lg:inline-block">
          Nieuwsbrief
        </a>
      ...
      {open && (
        <div className="fixed inset-0 top-[57px] z-40 flex flex-col gap-6 p-8 lg:hidden" style={{ background: '#091830' }}>
          ...
          <a href="#nieuwsbrief" onClick={() => setOpen(false)} className="mt-4 inline-block self-start rounded-full bg-lime px-6 py-3 font-body text-sm font-bold text-primary-foreground">
            Nieuwsbrief
          </a>
        </div>
      )}
